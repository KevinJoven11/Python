...
def quantum_teleportation(theta: float = np.pi/2, phi: float = np.pi/2, lam: float = np.pi/2):
    ...
    # Simulate the circuit usong qasm simulator
...
if __name__ == "__main__":
    print(f"Total count for teleported state is: {quantum_teleportation()}")
